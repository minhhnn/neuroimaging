...
if($normalise){
        ...
        &do_cmd("/opt/neuroimaging/scripts/mincnorm -clobber -cutoff $model_norm_thresh -threshold -threshold_perc $model_norm_thresh -threshold_blur $abs_value $resfile $nrmfile");
...
my ($abs_step_x, $abs_step_y, $abs_step_z);
$abs_step_x = abs($step_x * 4);
$abs_step_y = abs($step_y * 4);
$abs_step_z = abs($step_z * 4);

&do_cmd("mincblur -clobber -3dfwhm $abs_step_x $abs_step_y $abs_step_z $fitfile initmodel");
